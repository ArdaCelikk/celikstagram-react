    ...
    const onClickLikeBtn =async (e)=> {
        try {
            ...
            if(e.target.getAttribute("data-id")) {
                idElement =  e.target.getAttribute("data-id")
            } else if(e.target.parentElement.getAttribute("data-id")) {
                idElement = e.target.parentElement.getAttribute("data-id")
            } else {
                idElement = e.target.parentElement.parentElement.getAttribute("data-id")
            }
            const res = await axios.post("/post/like", {postID: idElement})
        ...
        <>
            <div className="flex justify-start mb-4 border-t border-gray-100">
                ...
                <div className="flex justify-end w-full mt-1 pt-2 pr-5">
                    ...
                    <span onClick={onClickLikeBtn} data-id={props.postID} className="transition ease-out duration-300 hover:bg-gray-50 bg-gray-100 h-8 px-2 py-2 text-center rounded-full text-gray-100 cursor-pointer">
            ...
            <div className="flex w-full border-t border-gray-100">
                <div className="mt-3 mx-5 flex flex-row text-xs">
                    <div className="flex text-gray-700 font-normal rounded-md mb-2 mr-4 items-center">Comments:<div className="ml-1 text-gray-400 text-ms"> {props.comments.length}</div></div>
                ...
                <div className="mt-3 mx-5 w-full flex justify-end text-xs">
                    <div className="flex text-gray-700  rounded-md mb-2 mr-4 items-center">Likes: <div className="ml-1 text-gray-400 text-ms"> {props.likes.length}</div></div>
                </div>
            </div>
        </>
    )
